...
public class JerryEnchant extends AbstractEnchantment implements ITickTask, MovementHandler {
    ...
    @SneakyThrows
    public JerryEnchant() {
        this.angle = NBSDecoder.parse(ThePit.getInstance().getClass().getClassLoader().getResourceAsStream("God.nbs"));
        this.demon = NBSDecoder.parse(ThePit.getInstance().getClass().getClassLoader().getResourceAsStream("Zeppeli.nbs"));
